...

{
  ...
  const dropperShadowGeometry = new THREE.BoxBufferGeometry(
    1 * ATOM_SIZE * ATOM_SCALE,
    1 * ATOM_SIZE * ATOM_SCALE,
    1 * ATOM_SIZE * ATOM_SCALE
  );
  const dropperShadow = [];
  ...
  for (let i = 0; i < MAX_SHADOW * MAX_SHADOW; i++) {
    dropperShadow[i] = new THREE.Mesh(
      dropperShadowGeometry,
      dropperShadowMaterial
    );
  }
  ...
  DROPPER.tryDrop = (position) => {
    ...
    if (DROPPER_SHADOW) {
      ...
      if (
        true ||
        (DROPPER_POUR == "default" && UI.selectedElement.pour) ||
        DROPPER_POUR == "pour"
      ) {
        let id = 1;
  ...
  const dropAtom = (
    ...
    justShow = false,
    shadowNumber = 0,
    yOverride = Math.floor(DROPPER_HEIGHT)
  ) => {
    if (!UI) return;
    ...
    if (
      ...
      !D1_MODE &&
      !DROPPER_OVERRIDE &&
      space.atom.element != Empty
    ) {
      if (atomType != space.atom.element)
        return dropAtom(x, y, z, yOffset + 1, justShow, shadowNumber);
    }
    if (!justShow) {
      ...
      dropperShadow[shadowNumber].position.set(
        x * ATOM_SIZE,
        alteredY * ATOM_SIZE,
        alteredZ * ATOM_SIZE
      );
